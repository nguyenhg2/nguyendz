    ...
    <div className="page">
      ...
      <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 28, alignItems: 'start' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1px solid var(--border)', padding: 24, position: 'sticky', top: 80 }}>
          ...
          <div style={{ display: 'flex', justifySelf: 'space-between', fontSize: 14, marginBottom: 8, color: 'var(--muted)' }}>
            <span style={{flex: 1}}>Tạm tính</span><span>{fmt(cartTotal)}</span>
          </div>
          <div style={{ display: 'flex', justifySelf: 'space-between', fontSize: 14, marginBottom: 12, color: 'var(--green)' }}>
            <span style={{flex: 1}}>Phí giao hàng</span><span>{cartTotal >= 500000 ? 'Miễn phí' : fmt(30000)}</span>
          </div>
          <div style={{ display: 'flex', justifySelf: 'space-between', fontSize: 18, fontWeight: 800, color: 'var(--rose)' }}>
            <span style={{flex: 1}}>Tổng cộng</span><span>{fmt(cartTotal + (cartTotal < 500000 ? 30000 : 0))}</span>
          </div>
        </div>
      </div>
    </div>
