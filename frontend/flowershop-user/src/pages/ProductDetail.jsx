...
export function ProductDetailPage() {
  ...
  if (loading) return <div style={{ padding: 100, textAlign: 'center' }}>Đang tải thông tin bó hoa...</div>;
      ...
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, marginBottom: 48 }}>
          ...
          <div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 8, fontWeight: 700, textTransform: 'uppercase' }}>Số lượng</div>
              <div className="qty-ctrl" style={{ display: 'flex', width: 'fit-content' }}>
                <button className="qty-btn" onClick={() => setQty(q => Math.max(1, q - 1))} style={{ padding: '8px 15px', border: '1px solid var(--border)', background: '#fff', cursor: 'pointer' }}>−</button>
                <input className="qty-num" value={qty} readOnly style={{ width: 50, textAlign: 'center', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', borderLeft: 'none', borderRight: 'none' }} />
                <button className="qty-btn" onClick={() => setQty(q => q + 1)} style={{ padding: '8px 15px', border: '1px solid var(--border)', background: '#fff', cursor: 'pointer' }}>+</button>
              </div>
            </div>

            {/* Nút hành động */}
            <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
              <button className="btn btn-primary" style={{ flex: 1, padding: '14px', justifyContent: 'center', fontSize: 16 }} onClick={() => addToCart(p, qty)}>
                🛒 Thêm vào giỏ
              </button>
              <button className="btn btn-outline" style={{ flex: 1, padding: '14px', justifyContent: 'center', fontSize: 16 }} onClick={() => { addToCart(p, qty); navigate('cart'); }}>
                ⚡ Mua ngay
              </button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              {[['🚚', 'Giao nhanh 2-4h'], ['🌸', 'Hoa tươi 100%'], ['🔄', 'Đổi trả miễn phí'], ['📞', 'Hỗ trợ 24/7']].map(([i, t]) => (
        ...
        {activeTab === 'desc' ? (
          <div style={{ lineHeight: 1.8, color: 'var(--text)', maxWidth: 800, marginBottom: 60 }}>
            ...
            <div style={{ marginTop: 24, padding: 20, background: 'var(--warm)', borderRadius: 12 }}>
              ...
              <ul style={{ paddingLeft: 20, fontSize: 14 }}>
                ...
                <li>Tặng kèm thiệp chúc mừng, banner miễn phí.</li>
          ...
          <div style={{ maxWidth: 800, marginBottom: 60 }}>
            ...
            {/* Danh sách các đánh giá từ database */}
