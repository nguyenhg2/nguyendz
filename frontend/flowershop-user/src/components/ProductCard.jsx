...
export default function ProductCard({ p, horizontal }) {
  ...
  if (horizontal) return (
    <div className="card" style={{ display: 'flex', gap: 0, cursor: 'pointer', opacity: stock === 0 ? 0.6 : 1 }} onClick={() => navigate('product', { id })}>
      ...
      <div style={{ padding: '12px 16px', flex: 1 }}>
        ...
        {stock === 0 && <div style={{ color: '#dc2626', fontSize: 12, fontWeight: 700, marginTop: 4 }}>Hết hàng</div>}
      </div>
    </div>
  );

  return (
    <div className="card" style={{ cursor: 'pointer', opacity: stock === 0 ? 0.6 : 1 }}>
      <div onClick={() => navigate('product', { id })}>
        <div className="product-img" style={{ position: 'relative', overflow: 'hidden', height: 180, background: 'var(--warm)' }}>
          ...
          <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
            {stock === 0 && <span className="badge" style={{ background: '#6b7280', color: '#fff' }}>Hết hàng</span>}
            {sale && sale < price && stock > 0 && <span className="badge badge-sale">Giảm giá</span>}
            {p.isNew && stock > 0 && <span className="badge badge-new">Mới</span>}
            {(p.soldQuantity > 50 || p.badge === 'hot') && stock > 0 && <span className="badge badge-hot">Bán chạy</span>}
          </div>
        </div>
        <div style={{ padding: '14px 16px' }}>
          ...
          {stock > 0 && <div style={{ fontSize: 12, color: 'var(--muted)' }}>Còn {stock} sản phẩm</div>}
        </div>
      </div>
